...
public partial class CanvasViewModel : ObservableObject
{
    ...
    // Removed local IsDynamic property in favor of GridSettings.IsDynamic
    ...
    public void SelectObject(object? obj)
    {
        ...
        else if (obj == null) // Canvas clicked
        {
            foreach (var n in Nodes) n.IsSelected = false;
            SelectedObject = this; // Select CanvasViewModel (Global Settings)
        }
    }
    ...
    public CanvasViewModel()
    {
        ...
        var node1 = new Node { Title = "Start", PositionX = 100, PositionY = 100, Shape = NodeShape.Rectangle, Color = "#1E1E2E" };
        var node2 = new Node { Title = "Process", PositionX = 400, PositionY = 200, Shape = NodeShape.Hexagon, Color = "#313244" };
        var node3 = new Node { Title = "End", PositionX = 700, PositionY = 100, Shape = NodeShape.Circle, Color = "#45475A" };
        ...
        Links.Add(new Link(node2, node3));
        ...
        // Subscribe to GridSettings changes
        GridSettings.PropertyChanged += OnGridSettingsChanged;
    }

    private void OnGridSettingsChanged(object? sender, System.ComponentModel.PropertyChangedEventArgs e)
    {
        if (e.PropertyName == nameof(GridSettings.IsDynamic))
        {
            if (!GridSettings.IsDynamic)
            {
                // Reset all dots to original position when switching to Static
        ...
        else if (e.PropertyName == nameof(GridSettings.DotSize) || e.PropertyName == nameof(GridSettings.Spacing))
        {
            // Regenerate grid when structural settings change
            GenerateWorldGrid();
            UpdateVisibleDots();
            InvalidateAll();
        }
    ...
    {
        // Helper to force redraw
        OnGravityFieldsChanged(new Rect(0,0, 10000, 10000)); 
    }
    ...
    private void UpdateGravityFields()
    {
        ...
        // diactivated: This logic was used to apply dynamic field effects to dots. 
        // It calculates the combined effect of all gravity fields and updates the dot's position and size.
        // Now it is controlled by the IsDynamic switch.
        if (GridSettings.IsDynamic)
        {
    ...
    {
        if (source == target) return; // Don't link to self

        // Check if link already exists
        if (Links.Any(l => (l.Source == source && l.Target == target) || (l.Source == target && l.Target == source)))
        {
            return;
        }
